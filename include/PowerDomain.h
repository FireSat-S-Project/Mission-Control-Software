 ...
 *
 *          Inactive subsystems receive ZERO electrical power through MOSFET
 *          hardware gating — not just software sleep mode. This eliminates
 *          standby leakage and reduces thermal load on the spacecraft bus.
 *
 * Pin mapping targets STM32H7 (flight). For STM32L4 / ESP32 prototype,
 * see docs/hardware_mapping.md for pin remap.
 */
...
/**
 * @brief  Initialise all gate pins and force safe state (all gates LOW).
 *         Must be called from main() before any task starts.
 */
void PowerManager_init(void);

/**
 * @brief  Transition to the requested power domain.
 *         Pure state-transition function — returns promptly.
 *         Callers requiring a stabilisation delay (e.g. Stirling cooler
 *         warm-up) must implement that delay in their own task context
 *         AFTER this function returns.
 * @param  domain  Target PowerDomain_t.
 */
void PowerManager_setDomain(PowerDomain_t domain);

/**
 * @brief  Return the currently active power domain.
 */
PowerDomain_t PowerManager_getCurrentDomain(void);

/**
 * @brief  Return instantaneous power draw for the current domain (Watts).
 */
